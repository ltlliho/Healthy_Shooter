namespace SpriteKind {
    export const bed_food = SpriteKind.create()
}
...
function asking () {
    story.spriteSayText(Me, " 你能幫我減肥嗎？拜託啦！", 15, 1, story.TextSpeed.Fast)
    story.showPlayerChoices("當然能！", "嗯…有點難度。", "先想一下，等我吃完這包薯片…")
    if (story.checkLastAnswer("當然能！")) {
        story.spriteSayText(Me, "太好了！", 7, 1, story.TextSpeed.VeryFast)
        start = 1
    } else if (story.checkLastAnswer("嗯…有點難度。") || story.checkLastAnswer("先想一下，等我吃完這包薯片…")) {
        story.spriteSayText(Me, "拜託嘛~", 1, 3, story.TextSpeed.VerySlow)
        asking()
    }
}
...
sprites.onOverlap(SpriteKind.Player, SpriteKind.bed_food, function (sprite, otherSprite) {
    if (start == 2) {
...
controller.right.onEvent(ControllerButtonEvent.Pressed, function () {
    if (start == 2) {
        if (info.score() < 5) {
            animation.runImageAnimation(
            Me,
            assets.animation`fatman-moveright0`,
            300,
            true
            )
        } else if (info.score() >= 5) {
...
story.startCutscene(function () {
    Me.setPosition(-10, 60)
    story.printText("從前從前，有個超級大胖子。", 80, 23, 15, 1, story.TextSpeed.Fast)
    animation.runImageAnimation(
    Me,
    ...
    story.spriteMoveToLocation(Me, 80, 60, 50)
    animation.runImageAnimation(
    Me,
    assets.animation`fatman-stop`,
    300,
    true
    )
    story.printText("別誤會，他以前可不是這樣！", 80, 23, 15, 1, story.TextSpeed.Fast)
    story.printText("他曾經是個帥氣逼人的小子。", 80, 23, 15, 1, story.TextSpeed.Fast)
    story.printText("可惜啊", 80, 93, 15, 1, story.TextSpeed.Fast)
    story.printText("壞習慣像小怪獸一樣", 80, 93, 2, 1, story.TextSpeed.Fast)
    story.printText("一個個纏上了他的生活", 80, 93, 2, 1, story.TextSpeed.Fast)
    story.printText("直到有一天", 80, 93, 15, 1, story.TextSpeed.Fast)
    animation.runImageAnimation(
    Me,
    assets.animation`fatman-moveright_stop`,
    300,
    true
    )
    mySprite = sprites.create(img`
        ...
        ..91dd6616696....6...
        .b911d1666696bbbbb6b.
        bb99999999966666666bb
        .bbbbbbbbbbbbbbbbbbb.
        .....................
        `, SpriteKind.Projectile)
    mySprite.setPosition(100, 59)
    story.printText("他在鏡子裡看到自己胖到不行", 80, 93, 15, 1, story.TextSpeed.Fast)
    story.printText("他決定找回帥氣的自己", 80, 93, 6, 1, story.TextSpeed.Fast)
    pause(100)
    sprites.destroyAllSpritesOfKind(SpriteKind.Projectile, effects.fire, 200)
    pause(200)
    animation.runImageAnimation(
    Me,
    assets.animation`fatman-stop`,
...
scene.setBackgroundImage(img`
    ...
    `)
story.startCutscene(function () {
    story.printCharacterText("使用 W, A, S, D 移動")
    story.printCharacterText("吃掉櫻挑可以讓你瘦下來")
    story.printCharacterText("吃掉3個以上櫻桃就可以 按空白鍵來加速")
    story.printCharacterText("使用Enter+W, A,S,或D 把敵人射倒")
    story.printCharacterText("避開所有壞習慣 和Pizza 不然你會變得更胖！")
    start = 2
})
pauseUntil(() => start == 2)
music.setVolume(50)
...
let Cherry = sprites.create(img`
    ...
    2 e e 2 2 2 e 2 5 4 2 c . . . . 
    c 2 e e e 2 2 4 2 2 2 2 . . . . 
...
game.onUpdateInterval(6000, function () {
    if (start == 2) {
        ...
        for (let index = 0; index < 2; index++) {
            water.setPosition(randint(1, 159), randint(10, 119))
